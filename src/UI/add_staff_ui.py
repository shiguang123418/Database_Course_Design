...
class EmployeeAddApp:
    def __init__(self):
        self.db = pymysql.connect(host="127.0.0.1", user="root",password="082415", database="y3")
        self.root = tk.Tk()
        self.root.title("添加员工数据")
        # 设置窗口大小和位置
    ...
    def create_widgets(self):
        ...
        labels = ['员工编号', '员工姓名', '员工性别', '员工年龄', '员工工龄', '身份证号码', '联系电话', '入职日期',
                  '工资', '健康码']
        self.entries = dict()
    ...
    def add_employee(self):
        ...
        insert_stmt = "INSERT INTO staff (员工编号, 员工姓名, 员工性别, 员工年龄, 员工工龄, 身份证号码, 联系电话, 入职日期, 工资, 健康码) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
        # 执行SQL语句
        try:
            cursor.execute(insert_stmt, data)
            self.db.commit()
            messagebox.showinfo("成功", "员工数据已成功添加！")
