...
class RefundPage(Window):
    ...
    def __create_widgets(self):
        # 标题
        self.tk_label_title = Label(self, text="交易信息", anchor="center", bootstyle="default inverse")
        self.tk_label_title.place(relx=0.5, rely=0.05, anchor='center')

        # 表格
        columns = ('交易流水号', '交易日期', '员工编号', '商品编号', '交易数量', '交易金额','会员卡卡号','类型')
        self.tree = Treeview(self, columns=columns, show='headings')
        for col in columns:
            self.tree.heading(col, text=col, anchor='center')
            self.tree.column(col, anchor='center', width=100)  # Set initial width
    ...
    def __populate_data(self):
        ...
        for col in self.tree['columns']:
            self.tree.heading(col, text=col, anchor='center')
            self.tree.column(col, width=max([len(str(item[0]))]) + 20)
