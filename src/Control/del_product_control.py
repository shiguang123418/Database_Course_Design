...
class Controller:
    ...
    def y1(self,evt):
        ...
        try:
            ...
            if results:
                # 商品存在，弹出确认删除窗口
                confirm = messagebox.askyesno(title="确认删除", message="商品存在，是否确认删除？")
                if confirm:
                    delete_sql = f"DELETE FROM goods WHERE 商品编号 = {id}"
                    cursor.execute(delete_sql)
                    coon.commit()
                    messagebox.showinfo(title="提示", message="商品删除成功")

            else:
                messagebox.showinfo(title="提示", message="没有该商品")
            # 关闭游标和数据库连接
            cursor.close()
            coon.close()
